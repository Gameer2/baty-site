// define `SYNTROPY_ASSET_PATH` as a SSOT
const OSS_FONTS_CDN = "https://excalidraw.nyc3.cdn.digitaloceanspaces.com/oss/";
const OSS_FONTS_FALLBACK = "/";

/**
 * Custom vite plugin for auto-prefixing `SYNTROPY_ASSET_PATH` woff2 fonts in `syntropy-app`.
 *
...
module.exports.woff2BrowserPlugin = () => {
  ...
  return {
    ...
    transform(code, id) {
      ...
      // but at the same time can't be easily prefixed with the `SYNTROPY_ASSET_PATH` only for the `syntropy-app`
      if (!isDev && id.endsWith("/excalidraw/fonts/fonts.css")) {
        return `/* WARN: The following content is generated during syntropy-app build */
      ...
      if (!isDev && id.endsWith("syntropy-app/index.html")) {
        return code.replace(
