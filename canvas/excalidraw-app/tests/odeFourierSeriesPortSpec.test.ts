...
import { ODE_FOURIER_SERIES_PORT_SPEC } from "../syntropy/portSpecs/odeFourierSeries";
...
// the partial sum S_N is sampled client-side from a0/an/bn — the same path the calculus spec uses,
// since the ODE/PDE Fourier-series method reuses the shared fourierSeries op.
...
describe("ODE_FOURIER_SERIES_PORT_SPEC", () => {
  beforeEach(() => {
  ...
  it("identifies the ode/fourier-series run-mode method", () => {
    expect(ODE_FOURIER_SERIES_PORT_SPEC.engineId).toBe("ode");
    expect(ODE_FOURIER_SERIES_PORT_SPEC.methodId).toBe("fourier-series");
    expect(ODE_FOURIER_SERIES_PORT_SPEC.executionMode).toBe("run");
    expect(ODE_FOURIER_SERIES_PORT_SPEC.pageStoreKey).toBe(
      "engine-lab:ode-fourier-series",
    );
  });

  it("declares the curve output first so the archetype is real-line", () => {
    expect(ODE_FOURIER_SERIES_PORT_SPEC.outputs[0].kind).toBe("curve");
    expect(archetypeFromSpec(ODE_FOURIER_SERIES_PORT_SPEC)).toBe("real-line");
  });

  it("compute() is a sync placeholder returning an empty (not-yet-run) result", () => {
    expect(ODE_FOURIER_SERIES_PORT_SPEC.compute({ f: "x" }).outputs).toEqual(
      {},
    );
  });

  it("computeRun samples S_N client-side from the coeffs and overlays f via sampleCurve", async () => {
    const result = await ODE_FOURIER_SERIES_PORT_SPEC.computeRun?.({
      f: "x",
      L: "pi",
      mode: "full",
      N: 8,
  ...
  it("surfaces the engine's failure and skips sampling when fourierSeries fails", async () => {
    casCallMock.mockReset();
