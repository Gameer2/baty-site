...
describe("RealLineNode", () => {
  ...
  it("renders a second continuous curve (e.g. the target function) as a dashed path, not dots", () => {
    const { container } = render(
      <RealLineNode
        nodeId={NODE_ID}
        spec={CURVE_SPEC}
    ...
    // Still exactly the 2 sample dots from `samples` — the overlay must not add more circles.
