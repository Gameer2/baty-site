...
// Run-mode Real-line spec: computeRun calls CAS.fourierSeries(f, "x", L, mode, N) for the
// numeric coefficient arrays (a0, an, bn), then samples the N-term partial sum S_N(x)
// client-side — a pure numeric eval mirroring CalculusSymbolic.partialSumValueAt (no nerdamer,
// so no second CAS call for the headline curve) — and overlays f via sampleCurve over the
// expansion interval. The first output is `curve` so the archetype is real-line; a0 (number)
// follows. Inputs mirror the page (fourier-series.js); the LaTeX series + step table stay on
// the page (v1). See the calculus plan Task 16.
export const FOURIER_SERIES_PORT_SPEC: PortSpec = {
  engineId: "calculus",
  ...
  executionMode: "run",
  pagePath: "/math-lab/engines/calculus/methods/fourier-series.html",
  pageStoreKey: "engine-lab:calculus-fourier-series",
