...
const RealLinePlot = ({
  ...
  const VIEW_H = 100;
  const PAD = 6;
  const { points, samples, overlay, rectangles, fillArea } = curve;
  if (points.length < 2) {
    return null;
  }
  ...
  // A second continuous curve over the same domain (e.g. the function a Fourier partial sum is
  // approximating) — its own dashed, dimmer path so it reads as "the target" behind the primary
  // curve, not competing with it the way plotting it as `samples` dots would at 100+ points.
  const overlayPath =
    overlay &&
    overlay
      .map((p, i) => `${i === 0 ? "M" : "L"} ${sx(p.x)},${sy(p.y)}`)
