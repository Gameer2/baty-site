...
export class SyntropyFontFace {
  ...
  constructor(family: string, uri: string, descriptors?: FontFaceDescriptors) {
    this.urls = SyntropyFontFace.createUrls(uri);

    const sources = this.urls
      .map((url) => `url(${url}) ${SyntropyFontFace.getFormat(url)}`)
      .join(", ");
  ...
  private static createUrls(uri: string): URL[] | DataURL[] {
    ...
    // absolute assets paths, which are found in tests and syntropy-app build, won't work with base url, so we are stripping initial slash away
    const assetUrl: string = uri.replace(/^\/+/, "");
    const urls: URL[] = [];

    if (typeof window.SYNTROPY_ASSET_PATH === "string") {
      const normalizedBaseUrl = this.normalizeBaseUrl(
        window.SYNTROPY_ASSET_PATH,
      );

      urls.push(new URL(assetUrl, normalizedBaseUrl));
    } else if (Array.isArray(window.SYNTROPY_ASSET_PATH)) {
      window.SYNTROPY_ASSET_PATH.forEach((path) => {
    ...
    urls.push(new URL(assetUrl, SyntropyFontFace.ASSETS_FALLBACK_URL));
