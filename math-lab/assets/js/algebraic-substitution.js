/* Algebraic-Substitution page wiring. All symbolic work lives in integration-advanced.js — this
   file only reads inputs, calls CAS.algebraicSubstitution, and renders the result. Structurally
   identical to u-substitution.js (same result shape: {ok, u, substitution, result, latex,
   verified, rejected, steps}) since IntegrationAdvanced.algebraicSubstitution was built to the
   same contract as CalculusSymbolic.uSubstitution. */
(function () {
  "use strict";
  ...
  const form = document.getElementById("algsubForm");
  ...
  function updateStartCheck() {
    ...
    startStatusText.textContent = "Integrand parses — press Derive to search for a radical to clear.";
    return compiled;
  }
  ...
  // The technique-specific fields (u, substitution) don't apply to a SymPy answer, so they're
  ...
  form.addEventListener("submit", (e) => {
    ...
    CAS.algebraicSubstitution(integrand, VARIABLE)
      .then((result) => {
        if (!result.ok) {
