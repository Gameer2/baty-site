/* Integration by Parts page wiring. All symbolic work lives in calculus-symbolic.js — this
   file only reads inputs, calls CalculusSymbolic.integrationByParts, and renders. Same
   derivation-ladder idiom as u-substitution.js: the step slider reveals the argument, not
...
(function () {
  ...
  const form = document.getElementById("ibpForm");
  ...
  const statDv = document.getElementById("statDv");
  ...
  function updateStartCheck() {
    ...
    startStatusText.textContent = "Integrand parses — press Derive.";
    return compiled;
  }
  ...
  function showRefused(result) {
    ...
    refusedTableBody.innerHTML = (result.rejected || [])
      ...
      .join("") || `<tr><td colspan="2">No candidate split could even be formed.</td></tr>`;
  }
  ...
  // The technique-specific fields (u, dv) don't apply to a SymPy answer, so they're
  ...
  form.addEventListener("submit", (e) => {
    ...
    CAS.integrationByParts(integrand, VARIABLE)
      .then((result) => {
        if (!result.ok) {
