/* Trigonometric Substitution page wiring. All symbolic work lives in calculus-symbolic.js —
   this file only reads inputs, calls CalculusSymbolic.trigSubstitution (via the CAS worker),
   and renders. Same derivation-ladder idiom as the other technique pages. */
(function () {
  "use strict";
  ...
  function updateStartCheck() {
    ...
    startStatusText.textContent = "Integrand parses — press Substitute.";
    return compiled;
  }
  ...
  function showRefused(result) {
    ...
    refusedTableBody.innerHTML = (result.rejected || [])
      ...
      .join("") || `<tr><td colspan="2">No recognised radical √(a²±x²) or √(x²−a²) is present — trig substitution does not apply.</td></tr>`;
  }
  ...
  function render(result, integrand) {
    ...
    statSub.textContent = result.substitution;
    statA.textContent = "a = " + result.a;
  ...
  // The trig-sub answers involve √(quadratic) and asin/asec, which blow up outside their
  // domains — keep the plot window inside the radical's valid region so the lines stay finite.
  function plot(integrand, antiderivative) {
    ...
    if (!f.ok || !F.ok) { Plotly.purge("fxPlot"); return; }

    const xs = [], fs = [], Fs = [];
    for (let i = 0; i <= 300; i++) {
      const x = -1.95 + (i / 300) * (1.95 - (-1.95));
      let yf, yF;
  ...
  // The technique-specific fields (substitution, a) don't apply to a SymPy answer, so they're
  ...
  form.addEventListener("submit", (e) => {
    ...
    if (submitBtn) submitBtn.textContent = "Substituting…";

    CAS.trigSubstitution(integrand, VARIABLE)
      .then((result) => {
        if (!result.ok) {
