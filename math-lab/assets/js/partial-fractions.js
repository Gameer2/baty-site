/* Partial Fractions page wiring. All symbolic work lives in calculus-symbolic.js — this
   file only reads inputs, calls CalculusSymbolic.partialFractions (via the CAS worker), and
   renders. Same derivation-ladder idiom as the other technique pages: the step slider reveals
   the argument, not successive approximations. */
(function () {
  "use strict";
  ...
  const statDen = document.getElementById("statDen");
  ...
  function updateStartCheck() {
    ...
    startStatusText.textContent = "Integrand parses — press Decompose.";
    return compiled;
  }
  ...
  function showRefused(result) {
    ...
    refusedTableBody.innerHTML = (result.rejected || [])
      ...
      .join("") || `<tr><td colspan="2">The integrand is not a rational function — partial fractions does not apply.</td></tr>`;
  }
  ...
  function render(result, integrand) {
    ...
    statNum.textContent = result.numerator;
    statDen.textContent = result.factoredDenominator || result.denominator;
  ...
  // The technique-specific fields (numerator, denominator, factoredDenominator) don't apply to a SymPy answer, so they're
  ...
  form.addEventListener("submit", (e) => {
    ...
    if (submitBtn) submitBtn.textContent = "Decomposing…";

    CAS.partialFractions(integrand, VARIABLE)
      .then((result) => {
        if (!result.ok) {
